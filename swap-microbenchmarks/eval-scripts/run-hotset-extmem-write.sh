...
for ((threads=start_threads; threads<=end_threads; threads*=2)); do
  ...
  LD_PRELOAD=/home/sepehr/uswap-dev/5.15-Uswap/src/libuswap-disklrulinux.so timeout 180s ./mmapbench /dev/null "$threads" 2 0 0 1 > "extmem-hotset-write-$threads-8-16.log"
done
